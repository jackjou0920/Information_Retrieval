...
class IndexLoader:
    def __init__(self, indexFile):
        ...
        f = open(indexFile, 'r')
        for line in f:
            # split only once and get the first token in term
            term = line.split(' ', 1)[0]
            # add new item with key=term and value={}(new dictionary) in index dictionary
            self.index[term] = {}
            # find all strings which match the (\d+):(\d+) in line
